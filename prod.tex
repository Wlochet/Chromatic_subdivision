...
In order to prove the lemma, we shall prove that $\chi(D[X^+(c)]) < \dr$ and $\chi(D[X^+(c)]) < \dr$ for all $c\in [\alpha_k]$.
To do so, we partition the set of arc of $D[X^+(c)]$ in $(A_0(c),A_1(c))$, where
\begin{itemize}
	\item $A_0(c)$ is the set of arcs of $D[X^+(c)]$ which ends belong to the same level, and
	\item $A_1(c)$ is the set of arcs of $D[X^+(c)]$ which ends belong to different levels.	
\end{itemize} 

Let $D^+_0(c)$  (resp. $D^+_1(c)$) be the spanning subdigraph of $D[X^+(c)]$ with arc set $A_0(c)$ (resp. $A_1(c)$).
In the next two claims, we bound the chromatic number of $D^+_0(c)$ and $D.^+_1(c)$
...
\begin{subproof}
Let $\phi_1$ be the colouring that assigns to all vertices of level $L_i$ the colour $i$ modulo $k$.

Let us show that $\phi_1$ is a proper colouring of $D^+_1(c)$. 
...
Claims~\ref{claim:D0} and \ref{claim:D1}, together with Lemma~\ref{lem:decomp} imply $\chi(D[X^+(c)]) \leq  \dr$ for all $c\in [\alpha_k]$.
