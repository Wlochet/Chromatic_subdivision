...
In order, to prove the lemma, we shall prove the chromatic numbers of $D[X^-]$ and $D[X^+]$ are at most $\beta_k/2$.
This yields the result as $\chi(D) \leq \chi(D[X^-]) +\chi(D[X^+])$.


We partition the set of arc of $D[X^+]$ in $(A_0(c),A_1(c), A_2(c))$, where
\begin{itemize}
	\item $A_0$ is the set of arcs of $D[X^+]$ which ends belong to the same level, and
	\item $A_1$ is the set of arcs of $D[X^+]$ which ends belong to different levels $i$ and $j$ with $ | i - j| < k$.	
	\item $A_2$ is the set of arcs of $D[X^+]$ which ends belong to different levels $i$ and $j$ with $ | i - j| \geq k$.
\end{itemize} 

For $i \in [3]$, let $D^+_i$ be the spanning subdigraph of $D[X^+]$ with arc set $A_i$.
In the next three claims, we bound the chromatic number of $D^+_0$, $D^+_1$ and  $D^+_2$.
...
is easy to see that $\phi_1$ is a proper colouring of $D^+_1$.
\end{proof}
...
\begin{subproof}
We will show that $D^+_2$ is acyclic with maximum out-degree at most $2k$.  This directly implies that  $D^+_2$ is $(k-1)$-degenerate, and so $\chi(D^+_2) \leq 2k+1$.

Let $x$ and $y$ be two adjacent vertices of $D^+_2$. Without loss of generality, we may assume that $x\in L_i$ and $y\in L_j$ for $i$ and $j$ such that $ j> i+k$.
...
In particular $C_{p-1}$ is the father of $C_p$. Since $y\in X^+$, then $C[y,t_{p-1,p}]$ has length at least $k$.
...
This means the only arcs of $D^+_2$ are the arcs $yx$ such that $C_x$ is an ancestor of $C_y$ and if $C_1 \dots C_p$ is the
sequence of ancestors from $C_x$ to $C_y$, then $C_1[x,t_{1,2}]$ has length less than $k$. 
In particular, $D^+_2$ is acyclic. Let us now show that is has out-degree at most $2k$.


Suppose for a contradiction that  $y$ has out-degree $2k+1$ in $D^+_2$. We can enumerate its out-neigbhours $x_1, x_2, \dots, x_{2k}$ such that $x_i$ is an ancestor of all $x_j$ for $i<j \leq 2k$. Let $C_1 \dots C_p$ be  the sequence such that $x_1\in C_1$ and $C_p = C_y$ and for all $l\in [p-1]$, $C_l$ is the father of $C_{l+1}$.
For all $i$, let $C_{l_i}$ be the cycle containing $x_i$.
%  and let$C_2$ be the cycle such that, $C_2$ belong to the path from $C_{x_1}$ to $C_y$ on the tree and $C_{x_k}$ is the father of $C_2$.
 %We proved that $C_{x_k}[t_{x_k},2]$ has size less than $k$ which means that $x_k$ belongs to the path $P$ along the tree from $x_1$ to $y$.
 ...
 Both $P[x_1, z]$ has length at least $k-1$ and $P[z, y]$ has length at least $k$. Hence the union of $(y,x_1) \odot P[x_1, z]$, $(y,x_k)\odot C_{l_{k+1}}[x_{k+1},z]$, and $P[z, y]$ is a subdivision of $P(k,1;k)$, a contradiction.
\end{subproof}
