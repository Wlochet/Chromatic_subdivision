...
\title{Internally disjoint directed paths in digraphs with large chromatic number}
...
\begin{abstract}
We prove that there exists a constant $\gamma_k$ such that every strongly connected digraph with chromatic number at least $k$ contains
two vertices joined $x$ and $y$ by three internally disjoint directed paths, one of which is from $x$ to $y$ and has length at least $k$ and one of which  is from $y$ to $x$ and has length at least $k$.
We also show that there exist strongly connected digraphs with arbitrarily large chromatic number such that between any two vertices $x$ and $y$, there is no three internally disjoint directed paths all from $x$ to $y$ and there is no four internally disjoint directed paths, two from $x$ to $y$ and two from $y$ to $x$.
\end{abstract}
...
There is a constant $\gamma_k$ such that if $D$ is a strong digraph with $\chi(D) > \gamma_k$, then $D$ contains a subdvision of $P(k,1;k)$.
\end{theorem}
...
internally disjoint from $C_1$. Hence the union of $P \odot yx$, $C_1[x,t_{1,2}]$ and $C_1[t_{1,2},x]$ is a subdivsion of  $P(k,1;k)$.
...
$C_1[x,t_{1,2}]$ has length less than $k$ , for otherwise the union of $P \odot yx$, $C_1[x,t_{1,2}]$ and $C_1[t_{1,2},x]$ would be a subdivision of  $P(k,1;k)$.
...
$P[z, y]$ is a subdivision of $P(k,1;k)$, a contradiction.
...
If $C_l=C_m$, then both $C_l[x,y]$ and $C_l[y,x]$ have length at least $7k$ because the subdipaths of  length $7k$ of $C_l$ are rainbow-coloured by $\phi$. Hence the union of those paths and $(x,y)$ is  a subdivision of $P(k,1;k)$, a contradiction.
...
$C_l[s_{l,m},x]$ are both longer than $k$. Therefore they form with $(x,y) \odot C_m[y,s_{l,m}]$ a subdivision of $P(k,1;k)$, a contradiction.
...
$(x,y) \odot C_m[y, s_{m,m'}] \odot P$, and $C_l[x,s_{l,l'}]$ is a subdivision of $P(k,1,;k)$, a contradiction.
\end{subproof}
...
Thus the union of $C[x,y]$, $P[x,y]$ and $C[y,x]$ is a subdivision of $P(k,1,;k)$, a contradiction.
...
and $P[v_{\ell_k}, v_f] \odot P_3 \odot P_5[s_{j'_3,j_3}, v]$ is a subdivision of $P(k,1;k)$,  a contradiction.
	...
	Hence the union of $P_1[u, v_{\ell_1}]\odot P[v_{\ell_1}, v_{\ell_k}]$, $P_3[u, s_{j'_3,j_3}]\odot P_6 \odot P_2$, and $P[v_{\ell_k}, v_f] \odot P_3[v_f,u]$,  is a subdivision of $P(k,1;k)$, a contradiction.
...
a subdivision of $P(k,1;k)$. So $C'$ is a directed cycle of length at least $8k$ which intersects every cycle of $\mathcal{C}$ along a 
