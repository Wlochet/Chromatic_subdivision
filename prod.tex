...
\begin{subproof}
We will show that $D^+_1(c)$ is $2k$ degenerate by showing that the maximum oudegree is smaller than $k$.  

Let $x$ and $y$ be two adjacent vertices of $D^+_1(c)$.
...
This means the only arcs of$D^+_1(c)$ are the arcs $yx$ such that $C_x$ is an ancestor of $C_y$ and if $C_1 \dots C_P$ is the
sequence of cycle from $C_x$ to $C_y$ on the tree, then $ \| C_1[x,t_{1,2}] \| < k$. 

Suppose $y$ has outdegree $k$ in $D^+_1(c)$. We can enumerate the outneigbhours $x_1, x_2, \dots, x_k$ such that
