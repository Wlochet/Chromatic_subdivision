...
Related questions can be looked at for other parameters than $\chi$. We refer the reader to \cite{AC+16} and \cite{CHLN16} for related questions.
...
In a digraph $D$, a \emph{directed path}, or \emph{dipath} in an oriented path where all the arcs are oriented in the same direction. 
...
\begin{proof}
First note that, since $D$ is strong,  so is $D_{\mathcal{C}}$. Suppose $\chi(D_{\mathcal{C}}) \geq 2k$, by Bondy's Theorem, there exists a directed cycle
...
complicated and because there exists arcs between cycle of the same component, bounding the chromatic number of the component,
which corresponds to Lemma \ref{lem:compo1}, is way more challenging. 
...
$C_2[t_{1,2},v]$ has length at least than $3k$.
...
dipath internaly disjoint from $C_1$ between two vertices at distance at least $7k$ of the cycle $C_1$ in $\mathcal{C}$, which is impossible. 
