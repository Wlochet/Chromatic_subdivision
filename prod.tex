...
Consider the colouring $c$ obtained by lemma \ref{lem:col-union-cycle}. It allow us 
...
Let $C_l$ be a cycle and $C_a$ it's father. let $Q_l$ be the subdipath of $C_l$ which contains
...
Suppose there exists a vertex $x$ such that $x \in Q^+_l$ and $x \in Q^-_s$ for some cycles $C_l$ and $C_s$ in some $L_j$.
If $x$ belongs to $L_j$ then $D$ contains a subdivision of $P(k,1;k)$
\end{lemma}

\begin{proof}
Suppose $x$ belongs to $L_j$, we can assume $x = l_{s,l}$.
Let $C_{l'}$ and $C_{s'}$ be the father of $C_l$ and $C_s$ (they can be the same). 
By definition of the $L_i$ there exists a dipath $P$ from $s_{l,l'}$ to $l_{s,s'}$ only going through cycles of $L_t$ for $t<j$.
...
Suppose there exists an arc between $x_1 \in C_{l_1}$ and $x_2 \in C_{l_2}$, then $x_1 \in Q_{l_1}$ and 
$x_2 \in Q_{l_2}$. 
\end{lemma}

Let $Q^+(r)$ (resp $Q^-(r)$) be the set of vertices belonging to some $Q^+_l$ (resp $Q^-_l$)  and is coloured $r$ in $c$. 
By lemmas \ref{lem:Qpos} and \ref{lem:Ql}, if for every $r$ we can colour the graph $D_r^+$ induced on $D$ by the vertices in 
...
\begin{itemize}
	\item $A_0$ is the set of arcs of $D_r^+$ which ends belongs to different levels.
	\item $A_1$ is the set of arcs of $D_r^+$ which ends belongs to the same levels.
\end{itemize} 
...
\begin{itemize}
	...
	\item For each vertex $x_j$ which correspond to a $s_i$ in $D$: the arc $x_{j-1}x_j$ correspond in $D$ to an arc arriving on 
		$p_i$, a vertex of $S_i$ and the arc $x_jx_{j+1}$ correspond to an arc leaving $l_i$, a vertex of $S_i$. Let $P_j$ be the dipath 
