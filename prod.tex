...
Take $D_{k,4}$ given by the previous theorem. Consider the digraph $D$ obtained from $D_{k,4}$ as follows: Add two new vertices $x$ and $y$
and the following edges: $xy$ and for each strong component of $D_{k,4}$, add one arc to $x$ and one arc from $x$. In $D_{k,4}$, every
directed cycle uses the arc $xy$, therefore $D$ does not contains $P(1,1;1,1)$ and $\chi(D) > k$. 
...
Let $C_l$ be a cycle and $C_a$ it's father. 
...
By lemmas \ref{lem:Qpos} and \ref{lem:Qneg}, if for every $r$ we can colour the graph $D_r^+$ induced on $D$ by the vertices in 
