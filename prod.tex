...
Let $D$ be a strongly connected digraph. If $\chi(D) >  (k+1)\dot (2k)$ then $D$ contains a subdivision of $P(1,1;k)$
\end{theorem}
...
component $S$ of $\mathcal{C}$. Then there is no path $P_S$ between $C_i$ and $C_j$ which uses only edges outside of $S$.  
\end{lemma}

\begin{proof}
We suppose there exists an $x-y$ dipath $P_S$ between $x \in C_i$ and $y \in C_j$ and we find a subdivision of $P(k,1;1)$. 
...
\begin{itemize}
	...
	Then the first path of the subdivision of $P(k,1;1)$ is $P \odot C_1[x_{1,2}, x] \odot P_S$, the second is 
...
$k$ colours. Let $S_1, S_2, \dots, S_r$ be the component containing cycles of $S$ in $\mathcal{C} \setminus{C}$. Since the $S$ is exactly the union of $S_i$ and $C$
then each $S_i$ has strictly less cycle than $S$. This means there exists a colouring $c_i$ using $k+1$ colours for each $S_i$.
...
of $P_S$, it is not possible. This means that we can consider that the vertices of $C$ receive the same colours in $c$ and in the $c_i$
...
\begin{itemize}
	...
	\item For each vertex $x_j$ which correspond to a component $S_i$ in $D$: the arc $x_{j-1}x_j$ corresponds in $D$ to an arc arriving on 
...
So $C'$ is a cycle longer than $2k$, intersecting cycle of $\mathcal{C}$ on at most one vertex, and which does not belong to $\mathcal{C}$, or it would be reduce
to one vertex in $D_{\mathcal{C}}$, which contradict the fact that $\mathcal{C}$ is maximal.
\end{proof}

So we can finally prove Theorem \ref{th:P11k}

\begin{proof}
${\cal C}$ be a maximal suitable collection of cycle in $D$. By lemma \ref{lem:compo1}, for each component $S_i$ of $\mathcal{C}$, there exists a colouring
...
Clearly, $c$ is a colouring of $D$ using $(k+1)\dot (2k)$ colours. 
\end{proof}
