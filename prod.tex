...
Observe moreover the digraph induced by 
...
For every colour $c\in [\alpha_k]$, let $X^+(c)$ be the set $X^+\cap \phi^{-1}(c)$ of vertices of $X^+$ coloured $c$, and $X^-(c)$ the set $X^-\cap \phi^{-1}(c)$ of vertices of $X^-$ coloured $c$.


\begin{claim}\label{lem:lvl}
Let $c\in [\alpha_k]$, and let $x$ and $y$ are vertices of $X^+(c)$ belonging to $L_i$ and $L_j$ respectively. 
...
\begin{lemma}
$\chi(D[X^+(c)]) < \dr$ for all $c\in [\alpha_k]$.
\end{lemma}

\begin{proof}

We partition the set of arc of $D[X^+(c)]$ in $(A_0,A_1)$, where
\begin{itemize}
	\item $A_0$ is the set of arcs of $D[X^+(c)]$ which ends belong to the same level, and
	\item $A_1$ is the set of arcs of $D[X^+(c)]$ which ends belong to different levels.	
\end{itemize} 

Let $D^+_0$  (resp. $D^+_1$) be the spanning subdigraph of $D[X^+(c)]$ with arc set $A_0$ (resp. $A_1$).


By Lemma \ref{lem:lvl}, $\chi(D^+_1) \leq k$ as the colouring that assigns to all vertices of level $L_i$ the colour $i$ modulo $k$ is proper. 

We will now prove that $\chi(D^+_0) \leq (2k)^{2k}$ by contradiction. 
Suppose it is not, it means that there exists a level $i$ such that the chromatic number of $\chi(D^+_0)$ in this level is greater 
...
By lemma \ref{lem:decomp} this means $\chi(D[X^+(c)]) \leq  \dr$ 
