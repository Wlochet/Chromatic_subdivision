...
Throughout this paper, a {\it colouring} of a digraph is a colouring of its underlying graph. Similarly,  the {\it chromatic number} of a digraph $D$, denoted by $\chi(D)$, is the chromatic number of its underlying graph. 
...
Recall that a {\it subdivision} of a digraph $F$ is a digraph that can be obtained from $F$ by replacing each arc $uv$ by a dipath from $u$ to $v$. Cohen et al.  \cite{CHLN16} conjecture that Bondy's theorem can be extended to all oriented cycles.
...
A {\it $p$-spindle} is the union of $k$ internally disjoint $(x,y)$-dipaths for some vertices $x$ and $y$. Vertex $x$ is said to be the {\it tail} of the spindle and $y$ its {\it head}.
...
\begin{proof}
Let $D_{k,4}$ be an acyclic digraph with chromatic number greater than $k$ and with every cycle having at least four blocks obtained by Theorem \ref{dkb}.
...
Consider the digraph $D$ obtained from $D_{k,4}$ as follows. Add a dipath  $P = (x_1,x_2, \dots , x_l,z,y_1,y_2,\dots, y_m)$ and the arc $(s_i,x_i)$ for all $i\in [l]$ and
...
Suppose now that $D$ has a $3$-spindle with tail $u$ and head $v$, and let $Q_1, Q_2, Q_3$ be its three $(u,v)$-dipaths. Observe that $u$ and $v$ are not vertices of $P$, because all vertices of this dipath have either in-degree $2$ or out-degree $2$. In $D$ each cycle with two blocks between vertices outside $P$ must use the arc $(x_l,z)$. The union of $Q_1$ and $Q_2$ form a cycle on two blocks, 
which means one of the two paths, say $Q_1$, contains $(x_l,z)$. But $Q_2$ and $Q_3$ also form a cycle on two blocks, but they cannot contain $(x_l,z)$, a contradiction.
\end{proof}
...
two $(x,y)$-dipaths, one of length $k_1$, the other of length $k_2$, and one $(y,x)$-dipath of length $k_3$.
...
By Proposition 5.11 in \cite{BoMu08},  there is a dipath $P$ in $D$ whose ends lie in $F$ but whose internal
...
For two sets $X,Y$ of vertices, a  {\it $(X,Y)$-dipath} or  {\it dipath from $X$ to $Y$} is a dipath $P$ such that $s(P)\in X$, $t(P)\in Y$, and no internal vertex is in $X\cup Y$.
...
any two distinct cycles of ${\cal C}$ intersect on at most one vertex. %When this intersection exists, we call it $x_{i,j}$.
...
Take a maximal nice collection ${\cal C}$ of directed cycles. 
We will prove that every component ${\cal S}$ of ${\cal C}$ induces a digraph $D[{\cal S}]$ on $D$ of bounded chromatic number. 
...
%By definition of $S$ there exists a sequence of directed cycles $C_1, C_2, \dots C_l$ such that $C_1 = C_i$, $C_l = C_j$ and $x_{s,s+1}$ exists for $s$ between $1$ and $l-1$.S
...
\begin{proof}
By induction on the number of directed cycles in ${\cal S}$. Let $C$ be a cycle of ${\cal S}$. There is no chord between $x$ and $y$ in $C$
...
$y$ be the initial and terminal vertex, respectively, of this path. Then the union of $C'[x,y]$,
...
any two distinct directed cycles $C_i,C_j\in\mathcal C$ intersect on  a subpath $P_{i,j}$ of order at most $k$.
We denote by $s_{i,j}$ (resp. $t_{i,j}$) the initial (resp. terminal) vertex of  $P_{i,j}$. 
%This notion generalises the notion of nice collection seen before. 
...
There exists a proper colouring $\phi$ of $\bigcup{\cal C}$ with $\alpha_k$ colours, such that, each subpath of length $7k$ of each directed cycle of $\mathcal{C}$ is rainbow-coloured.
\end{lemma}
...
%$C_2[t_{2,3}, s_{1,3}]$, and $C_2[s_{1,3}, t_{2,3}]$ is a $B(k,1;k)$-subdivision, a contradiction.
...
%	 Consequently, the union of $C_2[s_{1,2}, t_{2,3}]$, $C_3[t_{2,3}, s_{1,3}]\odot C_1[s_{1,3}, s_{1,2}]$, and $C_2[t_{2,3}, s_{1,2}]$ is a $B(k,1;k)$-subdivision, a contradiction.
%\end{itemize}
...
\begin{subproof}
To each vertex $v$ in $I^+(C_1)$ we associate the set $\Dis(v)$ of the lengths of the $C_j[t_{1,j},v]$ for all directed cycle $C_j\in {\cal C} \cap C_1$ containing $v$ such that $C_j[t_{1,j},v]$ has length at most $3k$.
...
than $\alpha_k$ colours such that, on each directed cycle, every subpath of length
...
Consider a rainbow-colouring of a subpath of length less than $7k$ of a directed cycle $C_1\in {\cal C}$.
...
Let $A$ be a connected component of $\bigcup{\cal C} - I(C_1)$. The coloured (by $\phi_1$) vertices of ${\cal C}\cap A$ are those of $({\cal C}\cap A)-A$. Hence, by Lemma \ref{lem:A}, they all belong to some directed cycle $C_j$ and so to the diptah $Q_j$ which has length at most $7k$.
...
The second step in the proof is the following lemma.
...
%all the vertices that are at distance at most $k$ from $P_{l,a}$ in the directed cycle underlying $C_l$.
...
%	Then we find a $B(k,1;k)$-subdivision between $y$ and $x_1$. 
